...
Public Class TcpClient
   ...
   Private c_getOK            As Boolean = False
   ...
   Private c_usbAddress       As System.Net.IPAddress = System.Net.IPAddress.Parse("172.22.11.2")
   Private c_openTimer        As Timer
   Private c_pingTimer        As Timer
   Private c_readTimer        As Timer
   Private c_receiveState     As ReceiveState = ReceiveState.None
   Private c_retryTimer       As Timer
   Private c_useUsbAddress    As Boolean = False
   ...
   Public Sub GetHostAddress()
      ...
      If c_useUsbAddress
         c_clientSocket.Open(c_usbAddress, m_dashOptions.robotPort)
      ElseIf IPAddress.TryParse(m_dashOptions.robotHost, Nothing) Then
         Dim hostIP As System.Net.IPAddress = System.Net.IPAddress.Parse(m_dashOptions.robotHost)
         c_clientSocket.Open(hostIP, m_dashOptions.robotPort)
      Else
         Dns.BeginGetHostAddresses(m_dashOptions.robotHost, AddressOf GetHostAddressCallback, Nothing)
      End If

      c_useUsbAddress = Not c_useUsbAddress
   End Sub
   ...
   Public Function GetPollInterval() As Double
      Return c_getTimer.Interval
   End Function
            ...
            Select command
               ...
               Case "GET"
                  ...
                  If parse.Count = 3
                     ...
                     If m_shellForm IsNot Nothing Then m_shellForm.InvokeFinshGet(mode)
               ...
               Case "PULL"
                  ...
                  If parse.Count = 2
                     m_robotSettingsCount = ConvertToInt(parse(0))
                     ReDim m_robotSettings(m_robotSettingsCount)

                     parse = parse(1).Split(",")

                     For i As Integer = 0 To m_robotSettingsCount - 1
                        m_robotSettings(i) = ConvertToDouble(parse(i))
                     Next

                     m_shellForm.InvokeFinishPull()

                  Else
                     ShowTraffic(TrafficFrom.Code, "Invalid PULL response")
                  End If
   ...
   Public Sub SendDashboardData(Optional skipSettings As Boolean = False)
      Dim i          As Integer
      Dim dash       As String = String.Empty

      If Not skipSettings
         If m_tcpClient IsNot Nothing Then m_tcpClient.SendMessage("SET:")
      End If
      ...
      Dim lastValue = m_dashValueCount - 1
      If skipSettings then lastValue = m_robotSetting1 - 1
   ...
   Public Sub SendMessage(message As String)
      If c_connected
         message = m_commandPrefix & message
         ShowTraffic(TrafficFrom.Client, message)
         c_clientSocket.Send(message)
      End If
   End Sub

   Public Sub SendPullRequest()
      If c_connected Then SendMessage("PULL:")
   End Sub
   ...
   Public Sub SetPingEnable(state As Boolean)
      If state
         If c_pingTimer Is Nothing
            c_pingTimer = New Timer(1000)
            c_pingTimer.AutoReset = False
            AddHandler c_pingTimer.Elapsed, New ElapsedEventHandler(AddressOf SendPing)
         End If

         c_pingTimer.Start()

      ElseIf c_pingTimer IsNot Nothing
         c_pingTimer.Stop()
      End If
   End Sub

   Private Sub SendPing()
      c_clientSocket.Send("Ping:")
      c_pingTimer.Start()
   End Sub
